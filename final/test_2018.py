...
test = pd.read_parquet(
    "W9/data2/test2/combined_mapped_data/combined_mapped_sampled_data.parquet")
...
y = test["Label"].apply(lambda label: label_mapping_2018.get(label, 'Unknown'))
...
tau_b = -0.99889741967896786
tau_m = 0.50
tau_u = -0.093889741967896786
...
pred_stage1_sample = np.where(
    proba_stage1_sample < tau_b, "Benign", "Attack").astype(object)
...
input_for_stage2_sample = np.column_stack(
    (x_s_stage2_features_sample, proba_stage1_sample.reshape(-1, 1)))
...
# Corrected: use scaled data
proba_1 = -stage1.decision_function(x_scaled_for_stage1)
# invert sign to act as anomaly score
pred_1 = np.where(proba_1 < tau_b, "Benign", "Attack").astype(object)
...
# Get original features of samples predicted as "Attack"
x_attack_original_np = x_np[attack_mask_stage1]

pred_2 = np.array([], dtype=object)  # Initialize pred_2

# Proceed only if there are "Attack" samples
if x_attack_original_np.shape[0] > 0:
    ...
    input_for_stage2 = np.column_stack(
        (x_attack_scaled_for_stage2, proba_1_for_attack_samples.reshape(-1, 1)))
...
def plot_confusion_matrix(y_true, y_pred, figsize=(7, 7), cmap="Blues", values=[-1, 1], labels=["Attack", "Benign"], title="", ax=None, metrics=False):
    ...
    all_unique_labels = sorted(
        list(set(y_true_series.unique()) | set(y_pred_series.unique())))
    ...
    # Use the passed labels for order
...
for cls in stage2_predictable_classes:
    if cls != 'Port Scan':  # Loại trừ 'Port Scan'
        final_evaluation_categories.append(cls)
final_evaluation_categories.append('Unknown')
final_evaluation_categories = sorted(
    list(set(final_evaluation_categories)))  # Đảm bảo duy nhất và sắp xếp
...
plot_confusion_matrix(y, y_pred, labels=final_evaluation_categories,
                      title="Confusion Matrix for 2018 Data (excluding Port Scan)")
plt.show()

print(classification_report(y, y_pred, digits=4,
      labels=final_evaluation_categories, zero_division=0))
