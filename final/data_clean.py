...
def plot_day(df):
    ...
    if 'Timestamp' in df.columns:
        ...
        for label in df.Label.unique():
            if label != 'Benign':
                df.loc[df["Label"] == label, 'Timestamp'].plot(
                    style='.', label=label)
        plt.legend()
        plt.show()
    else:
...
def clean_dataset(dataset, filetypes=['feather']):
    ...
    for file in os.listdir(f'{dataset}/original'):
        ...
        try:  # Add try block for individual file processing
            df = pd.read_csv(f"{dataset}/original/{file}",
                             skipinitialspace=True, encoding='latin')
            ...
            if has_timestamp:
                ...
                df['Timestamp'] = df['Timestamp'].apply(
                    lambda x: x +
                    pd.Timedelta(hours=12) if pd.notnull(
                        x) and x.hour < 8 else x
                )
                df = df.sort_values(by=['Timestamp'])
            else:
                print(
                    f"Warning: 'Timestamp' column not found in {file}. Skipping timestamp processing.")
            ...
            df[float_col] = df[float_col].apply(
