...
x_benign_train, _, _, _, x_benign_test, y_benign_test, x_malicious_train, y_malicious_train, x_malicious_test, y_malicious_test, attack_type_train, _, _ = util.load_data(
    clean_dir, sample_size=1948, train_size=n_benign_val, val_size=6815, test_size=56468)
train["stage2"]["x"], x_val, train["stage2"]["y"], y_val = train_test_split(
    x_malicious_train, y_malicious_train, stratify=attack_type_train, test_size=0.2, random_state=42, shuffle=True)

test['x'] = np.concatenate((x_benign_test, x_malicious_test))
test["y_n"] = np.concatenate(
    (y_benign_test, np.full(y_malicious_test.shape[0], -1)))
...
test["y_unknown"] = np.where(
    (test["y"] == "Heartbleed")
    | (test["y"] == "Infiltration")
    | (test["y"] == "FTP-BruteForce"), "Unknown", test["y"])
test["y_unknown_all"] = np.where(
    test['y_unknown'] == 'Benign', "Unknown", test['y_unknown'])
    ...
    return Pipeline(
        [
            ...
            ("pca", PCA(n_components=None, copy=True, whiten=False,
             svd_solver='auto', tol=0.0, iterated_power='auto', random_state=42)),
            ("ocsvm", OneClassSVM(kernel='rbf', degree=3, gamma='scale', coef0=0.0,
             tol=0.001, nu=0.5, shrinking=True, cache_size=200, verbose=True, max_iter=-1))
        ]
    ).set_params(**params)
...
curves_metrics_10k, summary_metrics_10k = util.evaluate_proba(
    val['ocsvm']['y'], score_val_10k)
...
curves_metrics_100k, summary_metrics_100k = util.evaluate_proba(
    val['ae']['y'], score_val_100k)
print("OCSVM (100k) Validation Summary:")
print(summary_metrics_100k)

#  Define Thresholds
quantiles = [0.995, 0.99, 0.975, 0.95, 0.935, 0.925,
             0.9, 0.85, 0.8, 0.75, 0.7, 0.65, 0.6, 0.55, 0.5]
print("Thresholds based on 10k model validation:")
print({(metric, fpr): t for metric, fpr, t in zip(summary_metrics_10k.metric,
      summary_metrics_10k.FPR, summary_metrics_10k.threshold)})
print(
    {q: np.quantile(score_val_10k[val["ocsvm"]["y"] == 1], q) for q in quantiles})

quantiles = [0.995, 0.99, 0.975, 0.95, 0.935, 0.925,
             0.9, 0.85, 0.8, 0.75, 0.7, 0.65, 0.6, 0.55, 0.5]
print("\nThresholds based on 100k model validation:")
print({(metric, fpr): t for metric, fpr, t in zip(summary_metrics_100k.metric,
      summary_metrics_100k.FPR, summary_metrics_100k.threshold)})
print(
    {q: np.quantile(score_val_100k[val["ae"]["y"] == 1], q) for q in quantiles})

#  Test
print("\nTesting OCSVM model (10k)...")
score_test_10k = -ocsvm_model_10k.decision_function(test['ocsvm_s'])
curves_metrics_test_10k, summary_metrics_test_10k = util.evaluate_proba(
    test["y_n"], score_test_10k)
# Print the summary
...
curves_metrics_test_100k, summary_metrics_test_100k = util.evaluate_proba(
    test["y_n"], score_test_100k)
# Print the summary
...
# Define Thresholds
fmacro, fweight, threshold_fscore, f_best = util.optimal_fscore_multi(
    val['stage2']['y'], y_proba_val_2, rf_model_baseline.classes_)
print(f_best["f1_weighted_threshold"])
y_pred_val_2 = np.where(np.max(y_proba_val_2, axis=1) > f_best["f1_weighted_threshold"], rf_model_baseline.classes_[
                        np.argmax(y_proba_val_2, axis=1)], 'Unknown')

# Test (Using Baseline RF)
print("\nTesting Stage 2 Random Forest (Baseline)...")
y_proba_test_2 = rf_model_baseline.predict_proba(test['stage2_s'])
y_pred_test_2 = np.where(np.max(y_proba_test_2, axis=1) > f_best["f1_weighted_threshold"], rf_model_baseline.classes_[
                         np.argmax(y_proba_test_2, axis=1)], 'Unknown')
...
threshold_b = -0.11819793702129796  # F8 100k
...
y_pred_2 = np.where(np.max(y_proba_2[y_pred == "Fraud"], axis=1) > threshold_m, train["stage2"]
                    ["y_n"].columns[np.argmax(y_proba_2[y_pred == "Fraud"], axis=1)], 'Unknown')
...
classes = ['Benign', '(D)DOS', 'Botnet', 'Brute Force',
           'Port Scan', 'Web Attack', 'Unknown']
util.plot_confusion_matrix(
    test['y_unknown'], y_pred, values=classes, labels=classes)
plt.show()

# Extension stage
threshold_u = -0.088640705317601636  # 0.7 100k
y_pred_3 = np.where(y_proba_1[y_pred == "Unknown"]
                    < threshold_u, "Benign", "Unknown")
...
print("Shapes:", proba_val.shape,
      val['stage2']['x_s'].shape, test['stage2_s'].shape)
...
print("With proba shapes:", train_with_proba.shape,
      val_with_proba.shape, test_with_proba.shape)
...
y_proba_val_2_extra_feature = rf_model_extra_feature.predict_proba(
    val_with_proba)
fmacro, fweight, threshold_fscore, f_best = util.optimal_fscore_multi(
    val['stage2']['y'], y_proba_val_2_extra_feature, rf_model_extra_feature.classes_)
print(f_best["f1_weighted_threshold"])
y_pred_val_2_extra_feature = np.where(np.max(y_proba_val_2_extra_feature, axis=1) > f_best["f1_weighted_threshold"], rf_model_extra_feature.classes_[
                                      np.argmax(y_proba_val_2_extra_feature, axis=1)], 'Unknown')

y_proba_test_2_extra_feature = rf_model_extra_feature.predict_proba(
    test_with_proba)
y_pred_test_2_extra_feature = np.where(np.max(y_proba_test_2_extra_feature, axis=1) > f_best["f1_weighted_threshold"], rf_model_extra_feature.classes_[
                                       np.argmax(y_proba_test_2_extra_feature, axis=1)], 'Unknown')
print({
...
threshold_b = -0.004015315009577358  # F2 100k
y_pred = np.where(y_proba_1 < threshold_b, "Benign", "Fraud").astype(object)
print(np.unique(y_pred, return_counts=True))

# threshold_m = 0.94 # See table above
threshold_m = 0.95  # See table above
y_pred_2 = np.where(np.max(y_proba_2[y_pred == "Fraud"], axis=1) > threshold_m, train["stage2"]
                    ["y_n"].columns[np.argmax(y_proba_2[y_pred == "Fraud"], axis=1)], 'Unknown')
print(np.unique(y_pred_2, return_counts=True))
...
threshold_u = -0.0006594446628764348  # 0.995 100k
y_pred_3 = np.where(y_proba_1[y_pred == "Unknown"]
                    < threshold_u, "Benign", "Unknown")
print(np.unique(y_pred_3, return_counts=True))
...
def generateConfusionGraphs(y_proba_2_new, threshold_m, include_metrics=False):
    ...
    confusion_1_binary = util.plot_confusion_matrix(test['y_n'], y_pred_1_n, values=[
                                                    1, -1], labels=["Benign", "Fraud"], title="Stage 1", ax=axs[0, 0])
    y_pred = np.where(y_proba_1 < threshold_b, "Benign", "Fraud")

    y_proba_2 = y_proba_2_new
    y_pred_2 = np.where(np.max(y_proba_2[y_pred == "Fraud"], axis=1) > threshold_m, train["stage2"]
                        ["y_n"].columns[np.argmax(y_proba_2[y_pred == "Fraud"], axis=1)], 'Unknown')
    confusion_2_multi = util.plot_confusion_matrix(
        test['y_unknown'][y_pred == "Fraud"], y_pred_2, values=classes, labels=classes, title="Stage 2", ax=axs[0, 1])
    ...
    confusion_12_multi = util.plot_confusion_matrix(
        test['y_unknown'], y_pred, values=classes, labels=classes, title="Stage 1&2 Combined", ax=axs[0, 2])
    mask = ((y_pred == "Unknown") & (test['y_unknown_all'] == "Unknown"))

    y_pred_3 = np.where(y_proba_1[y_pred == "Unknown"]
                        < threshold_u, "Benign", "Unknown")
    y_pred_3_n = np.where(y_proba_1[mask] < threshold_u, 1, -1)
    confusion_3_multi = util.plot_confusion_matrix(
        test['y_unknown'][y_pred == "Unknown"], y_pred_3, values=classes, labels=classes, title="Extension Multi-Class", ax=axs[1, 0])
    confusion_3_binary = util.plot_confusion_matrix(test['y_n'][mask], y_pred_3_n, values=[
                                                    1, -1], labels=["Benign", "Zero-Day"], title="Extension Binary", ax=axs[1, 1])
    ...
    confusion_123_multi = util.plot_confusion_matrix(
        test['y_unknown'], y_pred, values=classes, labels=classes, title="Stages 1,2 & Extension Combined", ax=axs[1, 2])
...
generateConfusionGraphs(y_proba_test_2, 0.95, True)
plt.show()
