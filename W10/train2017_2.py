...
output_dir = pathlib.Path("W10/models1")
...
ocsvm_param_dist = {
    ...
    'ocsvm__gamma': loguniform(3e-2, 9e-2), # Sample from log-uniform distribution between 
    'ocsvm__nu': loguniform(1e-4, 3e-4), # Sample from log-uniform distribution between 
...
n_iterations = 100 # Number of parameter settings that are sampled. Adjust as needed.
...
# Fit RandomizedSearchCV (same logic as GridSearchCV fit)
...
print("\nRandomizedSearchCV for OCSVM finished.") # <-- Changed
...
# Get the best pipeline found by RandomizedSearchCV (same as before)
...
print("Saved stage1_ocsvm_model_10k.p (Pipeline)")
# f = open("W10/models1/stage1_ocsvm_model_10k.p","rb")
...
clf_ocsvm.fit(train['ae']['x_s'], train['ae']['y']) # Fit on benign, score using validation labels
...
# f = open("W10/models1/baseline_rf.p","rb")
...
threshold_b = -0.14713456462666652 # See tables above
y_pred = np.where(y_proba_1 < threshold_b, "Benign", "Fraud").astype(object)
print(np.unique(y_pred, return_counts=True))
...
threshold_m = 0.99 # See table above
...
# Extension stage
threshold_u = -0.0610887238591802 # See table above
...
threshold_m = 0.92 # See table above
y_pred_2 = np.where(np.max(y_proba_2[y_pred == "Fraud"], axis=1) > threshold_m, train["stage2"]["y_n"].columns[np.argmax(y_proba_2[y_pred == "Fraud"], axis=1)], 'Unknown')
print(np.unique(y_pred_2, return_counts=True))
